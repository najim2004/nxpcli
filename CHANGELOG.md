...
- Template string escaping issues in `globalErrorHandler` and `requestLogger` templates
- Removed `String.raw` usage that was causing incorrect backslash escaping in generated code

## [1.4.2] - 2025-11-30

### Fixed

- TypeScript error in `globalErrorHandler` (missing `next` parameter)
- Updated `globalErrorHandler` return type to `void` for `ErrorRequestHandler` compatibility
